...
public class AllIn1SpriteShaderStencil : ShaderGUI
{
    ...
    private UnityEngine.Rendering.StencilOp stencilOp;
    private UnityEngine.Rendering.CompareFunction stencilComp;
    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        ...
        GenericEffect(materialEditor, properties, style, "Shine", "SHINE_ON", 129, 134);
        GenericEffect(materialEditor, properties, style, "Contrast & Brightness", "CONTRAST_ON", 148, 149);
        GenericEffect(materialEditor, properties, style, "Alpha Cutoff", "ALPHACUTOFF_ON", 70, 70);
        GenericEffect(materialEditor, properties, style, "Alpha Round", "ALPHAROUND_ON", 140, 140);
        ...
        if (toggle)
        {
            ...
            EditorGUILayout.BeginVertical(style);
            {
                ...
                if (toggle2)
                {
                    ...
                    for (int i = 142; i < 145; i++) materialEditor.ShaderProperty(properties[i], properties[i].displayName);
                }
                ...
                if (toggle2)
                {
                    ...
                    for (int i = 145; i < 148; i++) materialEditor.ShaderProperty(properties[i], properties[i].displayName);
                }
    ...
    private void Hologram(MaterialEditor materialEditor, MaterialProperty[] properties, GUIStyle style, string inspector, string keyword, int first, int last)
    {
        ...
        if (toggle)
        {
            ...
            if (first > 0)
            {
                ...
                {
                    for (int i = first; i <= last; i++) materialEditor.ShaderProperty(properties[i], properties[i].displayName);
                    materialEditor.ShaderProperty(properties[136], properties[136].displayName);
                }
                EditorGUILayout.EndVertical();
            }
        }
    ...
    private void Gradient(MaterialEditor materialEditor, MaterialProperty[] properties, GUIStyle style, string inspector, string keyword)
    {
        ...
        if (toggle)
        {
            ...
            EditorGUILayout.BeginVertical(style);
            {
                ...
                MaterialProperty gradIsRadial = properties[139];
                if (gradIsRadial.floatValue == 1)
                {
                    ...
                    materialEditor.ShaderProperty(properties[137], properties[137].displayName);
                }
                else
                {
                    ...
                    if (!simpleGradToggle) materialEditor.ShaderProperty(properties[137], properties[137].displayName);
